...
for taxiway in runways:
    ...
    folium.PolyLine(locations=taxiway, color='green', weight=2.5, opacity=1).add_to(m)
...
for taxiway in taxiways:
    # Opción 1: Usar directamente taxiway
    folium.PolyLine(locations=taxiway, color='blue', weight=2.5, opacity=1).add_to(m)
...
for taxiway in aprons:
    # Opción 1: Usar directamente taxiway
    folium.PolyLine(locations=taxiway, color='orange', weight=2.5, opacity=1).add_to(m)
    

# HTML para la leyenda
legend_html = '''
    <div style="position: fixed; 
                top: 200px; left: 200px; width: 120px; height: 80px; 
                background-color: white; z-index:9999; font-size:14px; 
                padding: 5px; border-radius:5px; box-shadow:2px 2px 6px rgba(0,0,0,0.5);">
        <span style="background-color: green; border-radius: 50%; width: 15px; height: 15px; display: inline-block; margin-right: 5px;"></span> Runways<br>
        <span style="background-color: blue; border-radius: 50%; width: 15px; height: 15px; display: inline-block; margin-right: 5px;"></span> Taxiways<br>
        <span style="background-color: orange; border-radius: 50%; width: 15px; height: 15px; display: inline-block; margin-right: 5px;"></span> Aprons
    </div>
'''
